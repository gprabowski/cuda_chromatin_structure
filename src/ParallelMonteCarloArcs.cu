...
__device__ float calcScoreHeatmapSingleActiveRegion(
    ...
    for (int i = st; i <= end; ++i) {
        if (abs(i-moved) >= heatmapDiagonalSize) {
            if ((helper = heatmap_dist[i * heatmapSize + moved]) < 1e-6) continue;	// ignore 0 values
            //no warp divergence as all threads are from the same warp so the warpIdx will evaluate the same
            subtractFromVector3(temp_one, (i != warpIdx) ? *(clusters_positions + i) : curr_vector,
    ...
    while(true) {
        ...
        for(int i = 0; i < N; ++i) {
            ...
            if((score_curr <= score_prev)
               ...
               || ( T > 0.0f && withChance(settings.tempJumpScaleHeatmap * expf(-settings.tempJumpCoefHeatmap * (score_curr/score_prev) / T), &localState))) {
        ...
        #define FULL_MASK 0xffffffff
            ...
            if(calcScoreHeatmapActiveRegion(-1, clusters_positions, heatmap_chromosome_boundaries, 
                heatmap_dist, heatmapSize, heatmapDiagonalSize, activeRegionSize, chromosomeBoundariesSize, curr_vector, warpIdx) <
                calcScoreHeatmapActiveRegion(-1, clusters_positions, heatmap_chromosome_boundaries, 
                heatmap_dist, heatmapSize, heatmapDiagonalSize, activeRegionSize, chromosomeBoundariesSize, *(clusters_positions + warpIdx), warpIdx)) {
...
float LooperSolver::ParallelMonteCarloHeatmap(float step_size) {
	...
	gpuErrchk(cudaMalloc((void**)&devStates, 32 * active_region.size() * sizeof(curandState)));

    // cuRand initialization
	setupKernel<<< active_region.size() / 2, 64 >>>(devStates, time(NULL));
    ...
    MonteCarloHeatmapKernel<<<active_region.size(), 256>>>(
        devStates,
