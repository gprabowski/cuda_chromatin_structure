...
{ 
 ...
 // printf("* %f %f %f %lf %d %lf\n", Eo, r1, r2, residual, n_list, sqrt( (double) residual*2.0 / ((double) n_list) ));
  residual = Eo - sqrt(r1) - sqrt(r2) - omega*sqrt(rlow);
  if (residual < 1e-6) *rmsd = 0.0;
  else *rmsd = sqrt( (double) residual*2.0 / ((double) n_list) );
}
